...
try {
    // 1. Create the new custom exam entry with NULL for lesson_id and topic_id
    ...
    foreach ($source_lessons as $source) {
        ...
        if ($question_count > 0) {
            $fetch_q_stmt = $conn->prepare("SELECT topic_id, question, options, answer, explanation FROM questions WHERE lesson_id = ? ORDER BY RAND() LIMIT ?");
            ...
            while ($q_row = $questions_result->fetch_assoc()) {
                $insert_q_stmt->bind_param("iiiissss",
                    $new_exam['subject_id'],
                    $source_lesson_id, // Keep original lesson_id for the question
                    $q_row['topic_id'], // Keep original topic_id for the question
                    $new_exam_id,
                    $q_row['question'],
                    $q_row['options'],
                    $q_row['answer'],
